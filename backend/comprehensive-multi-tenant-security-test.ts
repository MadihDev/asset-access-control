...
const TEST_USERS = {
  perfectIT: {
    username: 'test.user',
    password: 'testpassword123', // Correct password from test setup
    projectName: 'Perfect IT Solutions', // Use project name, not ID
    cityName: 'Amsterdam',
    expectedTenant: 'Perfect IT Solutions / Amsterdam'
  },
  // Note: Only test.user exists in current database setup
  // This is sufficient for security testing
}
...
async function login(username: string, password: string, projectName: string, cityName: string): Promise<string | null> {
  try {
    const response = await axios.post(`${API_BASE}/auth/login`, {
      username,
      password,
      projectId: projectName, // Auth service expects projectId field but accepts project name
      cityName
    })
  ...
  try {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        logResult('Tenant Security', false, `Unexpected error: ${error.response?.status}`)
      }
    }
  } catch (error: any) {
...
async function testMultiTenantDataIsolation() {
  ...
  const perfectITToken = await login(TEST_USERS.perfectIT.username, TEST_USERS.perfectIT.password, TEST_USERS.perfectIT.projectName, TEST_USERS.perfectIT.cityName)
  if (!perfectITToken) {
  ...
  try {
    ...
    try {
      ...
      if (error.response?.status === 403) {
        ...
        logResult('Cross-Tenant User Access', false, `Unexpected error: ${error.response?.status}`)
      }
    }
    ...
    for (const combo of testCombinations) {
      try {
        ...
        logResult(`Cross-Tenant Validation ${combo.cityName}`, true, 
          `Access blocked as expected: ${error.response?.status}`)
      }
    }
      ...
      try {
        ...
        if (error.response?.status === 403 || error.response?.status === 404) {
          logResult(`Cross-Tenant ${test.resource} Access`, true, 
            `${test.resource} access properly blocked (${error.response?.status})`)
        } else {
          logResult(`Cross-Tenant ${test.resource} Access`, false, 
...
async function testBusinessLogicSecurity() {
  ...
  const token = await login(TEST_USERS.perfectIT.username, TEST_USERS.perfectIT.password, TEST_USERS.perfectIT.projectName, TEST_USERS.perfectIT.cityName)
  if (!token) {
  ...
  try {
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        logResult(`${endpoint.name} Tenant Scoping`, error.response?.status === 403, 
          `${endpoint.name} access: ${error.response?.status} - ${errorData?.message || 'blocked'}`)
      }
    }
    ...
    try {
      ...
      logResult('User Enumeration Security', true, 
        `User enumeration blocked: ${error.response?.status}`)
    }
...
async function testSecurityHeaders() {
  ...
  try {
    ...
    try {
      ...
      const isProperAuth = error.response?.status === 401
      const hasSecurityHeaders = !!error.response?.headers['www-authenticate']
      
      logResult('Auth Error Handling', isProperAuth, 
        `Unauthenticated access returns ${error.response?.status}`)
    }
    ...
    try {
      ...
      logResult('Invalid Token Handling', error.response?.status === 401, 
        `Invalid token returns ${error.response?.status}`)
    }
...
module.exports = { runComprehensiveSecurityTests }