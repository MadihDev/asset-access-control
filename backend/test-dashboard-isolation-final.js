...
async function testDashboardTenantIsolation() {
  ...
  try {
    ...
    if (!perfectitSeesOnlyOwnData) {
      console.log(`   ❌ PerfectIT sees foreign project data:`, perfectitLogProjectCities.filter(id => id !== perfectitProjectCityId));
    }
    ...
    if (!acmeSeesOnlyOwnData) {
      console.log(`   ❌ Acme sees foreign project data:`, acmeLogProjectCities.filter(id => id !== acmeProjectCityId));
    }
